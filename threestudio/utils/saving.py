...
def calc_metrics(gt_img,comp_img,mask,gt_mask,criterions,result_dict,type="rgb",img_id=0,use_mask=True,use_scale=True):
    img_dict = {}
    # mask_inve =  #.repeat(3,axis=2)
    mask = mask>128
    gt_mask= gt_mask>128
    # mask = mask.astype(bool)
    # gt_mask = gt_mask.astype(bool)
    if gt_img.dtype=='uint8':
        gt_img = gt_img/255.0
    if comp_img.dtype=='uint8':
        comp_img = comp_img/255.0

    if use_mask:
        mask_intersect = mask * gt_mask
        comp_img_masked = np.ma.masked_array(comp_img,1-mask_intersect)
        gt_img_masked = np.ma.masked_array(gt_img,1-mask_intersect)
        scale =gt_img_masked.mean()/comp_img_masked.mean()
        # print(f"scale:{scale}")
        if not use_scale:
            scale=1

        comp_img = comp_img*mask*scale +(1-mask)
        gt_img = gt_img*gt_mask  +(1-gt_mask)

        # cv2.imwrite("temp_gt.png",gt_img*255)
        # cv2.imwrite("temp_comp.png",comp_img*255)
        gt_img = np.concatenate([gt_img,gt_mask[...,:1]],axis=-1)
        comp_img = np.concatenate([comp_img,mask[...,:1]],axis=-1)
        img_dict[f"gt_{type}_{img_id}"] = (gt_img*255)
        img_dict[f"comp_{type}_{img_id}"] = (comp_img*255)

        gt_img,comp_img,mask,gt_mask = (
            torch.as_tensor(gt_img, dtype=torch.float32).permute(2,0,1).unsqueeze(0),
            torch.as_tensor(comp_img, dtype=torch.float32).permute(2,0,1).unsqueeze(0),
            torch.as_tensor(mask, dtype=torch.float32).permute(2,0,1).unsqueeze(0),
            torch.as_tensor(gt_mask, dtype=torch.float32).permute(2,0,1).unsqueeze(0),
        )
        
        for name,criterion in criterions.items():
            criteria_name = f"{type}_{name}"
            if criteria_name not in result_dict.keys():
                result_dict[criteria_name]=[]
            value = criterion(gt_img, comp_img).flatten().detach().item()
            result_dict[criteria_name].append(value)
            # print(f"{criteria_name} loss for [{scene}] with [{model}]: {value}")
    else:
        scale =gt_img.mean()/comp_img.mean()
        gt_img = np.concatenate([gt_img,gt_mask[...,:1]],axis=-1)
        comp_img = np.concatenate([comp_img*scale,mask[...,:1]],axis=-1)
        img_dict[f"gt_{type}_{img_id}"] = (gt_img*255)
        img_dict[f"comp_{type}_{img_id}"] = (comp_img*255)
...
def calc_metric_imgs_(
    ...
    vis_img_dir = img_dir+"_vis"
    # if os.path.isdir(vis_img_dir):
    ...
    for f in os.listdir(img_dir):
        if matcher.search(f):
            jsons.append(f)
    jsons = sorted(jsons, key=lambda f: int(matcher.search(f).groups()[0]))
    result_dict={}

    for idx in range(len(jsons)):
        with open(os.path.join(img_dir, jsons[idx]), "r") as f:
            img_dict = json.load(f)
        img_id = int(jsons[idx].replace('.json',''))
        print(f"image_id: {img_id}")
        img = cv2.imread(os.path.join(img_dir, jsons[idx].replace('.json','.png'))) 
        vis_img_fname = os.path.join(vis_img_dir, jsons[idx].replace('.json','.png'))
        vis_img_dict = {}
        if "gt_rgb" in img_dict.keys():
            gt_rgb = img[:,img_dict["gt_rgb"]['start']:img_dict["gt_rgb"]['end']]# (H,W,3)
            comp_rgb = img[:,img_dict["comp_rgb"]['start']:img_dict["comp_rgb"]['end']]# (H,W,3)
            
            comp_mask = img[:,img_dict["opacity"]['start']:img_dict["opacity"]['end']] # (H,W,3)
            gt_mask = img[:,img_dict["mask"]['start']:img_dict["mask"]['end']] # (H,W,3)

            imgs_dict = calc_metrics(gt_rgb,comp_rgb,comp_mask,gt_mask,criterions, result_dict,type=f"rgb",img_id=img_id,use_scale=use_scale)
            vis_img_dict.update(imgs_dict)
        
            # if img_id%10==0:
            if "comp_kd" in img_dict.keys() and "gt_albedo" in img_dict.keys() :
                gt_albedo = img[:,img_dict["gt_albedo"]['start']:img_dict["gt_albedo"]['end']]# (H,W,3)
                if "gt_glossy" in img_dict.keys() and use_glossy:
                    gt_glossy = img[:,img_dict["gt_glossy"]['start']:img_dict["gt_glossy"]['end']]# (H,W,3)
                    gt_albedo = (gt_albedo/255.0 + gt_glossy/255.0)
                    gt_albedo = gt_albedo/gt_albedo.max()
                comp_albedo = img[:,img_dict["comp_kd"]['start']:img_dict["comp_kd"]['end']]# (H,W,3)

                imgs_dict = calc_metrics(gt_albedo,comp_albedo,comp_mask,gt_mask,criterions,result_dict,type=f"albedo",img_id=img_id,use_scale=use_scale)
                vis_img_dict.update(imgs_dict)
            
            if "comp_normal" in img_dict.keys():
                comp_normal = img[:,img_dict["comp_normal"]['start']:img_dict["comp_normal"]['end']]# (H,W,3)
                vis_img_dict[f"comp_normal_{img_id}"] = np.concatenate([comp_normal,comp_mask[...,:1]],axis=-1) 
            
            if "env_light" in img_dict.keys():
                env_light = img[:,img_dict["env_light"]['start']:img_dict["env_light"]['end']]# (H,W,3)
                vis_img_dict[f"env_light_{img_id}"] = env_light

            if "roughness" in img_dict.keys():
                roughness = img[:,img_dict["roughness"]['start']:img_dict["roughness"]['end']]# (H,W,3)
                vis_img_dict[f"roughness_{img_id}"] = np.concatenate([roughness,comp_mask[...,:1]],axis=-1) 
            
            if "metallic" in img_dict.keys():
                metallic = img[:,img_dict["metallic"]['start']:img_dict["metallic"]['end']]# (H,W,3)
                vis_img_dict[f"metallic_{img_id}"] = np.concatenate([metallic,comp_mask[...,:1]],axis=-1)

            if "occlusion" in img_dict.keys():
                occlusion = img[:,img_dict["occlusion"]['start']:img_dict["occlusion"]['end']]# (H,W,3)
                vis_img_dict[f"occlusion_{img_id}"] = np.concatenate([occlusion,comp_mask[...,:1]],axis=-1)
            
        else:
            for name,criterion in criterions.items():
                criteria_name = f"albedo_{name}"
                result_dict[criteria_name].append(result_dict[criteria_name][-1])


        for name,img in vis_img_dict.items():
            cv2.imwrite(os.path.join(vis_img_dir,f"{name}.png"),img)
...
# def calc_metrics(gt_img,comp_img,mask,gt_mask,criterions,result_dict,type="rgb",use_mask=True,use_scale=True):
#     if use_mask:
#         # mask_inve =  #.repeat(3,axis=2)
#         mask = mask>128
#         gt_mask= gt_mask>128
...
#         scale =gt_img.mean()/comp_img.mean()
#         gt_img,comp_img = (
#             torch.as_tensor(gt_img/255., dtype=torch.float32).permute(2,0,1).unsqueeze(0),
#             torch.as_tensor(comp_img/255., dtype=torch.float32).permute(2,0,1).unsqueeze(0)*scale,
#         )
#         for name,criterion in criterions.items():
#             criteria_name = f"{type}_{name}"
#             if criteria_name not in result_dict.keys():
...
#             calc_metrics(gt_rgb,comp_rgb,comp_mask,gt_mask,criterions, result_dict,type="rgb",use_scale=use_scale)
...
#             calc_metrics(gt_albedo,comp_albedo,comp_mask,gt_mask,criterions,result_dict,type="albedo",use_scale=use_scale)
...
class SaverMixin:
    ...
    def get_image_grid_(self, imgs, align):
        ...
        cols = []
        imgs_dict = {} # a dict indicated the start/end position of each image
        pos_ptr = 0
        for idx,col in enumerate(imgs):
            ...
            next_pos_ptr = pos_ptr + cols[-1].shape[1]
            imgs_dict[col["name"]] = {"idx":idx,"start":pos_ptr,"end":next_pos_ptr}
        ...
        if align != "ori":
            ...
            for i in range(len(cols)):
                if cols[i].shape[0] != h or cols[i].shape[1] != w:
                    ...
                    next_ptr = pos_ptr + cols[i].shape[1]
                    name = [name for name in imgs_dict.keys() if imgs_dict[name]["idx"] == i][0]
                    imgs_dict[name]={"idx":i,"start":pos_ptr,"end":next_ptr}
                    pos_ptr = next_ptr
        
        return np.concatenate(cols, axis=1), imgs_dict
    ...
    def save_image_grid(
        ...
        save_path = self.get_save_path(filename)
        img,imgs_dict = self.get_image_grid_(imgs, align=align)
        ...
        cv2.imwrite(save_path, img)
        with open(self.get_save_path(filename.replace('.png', '.json')), "w") as f:
            json.dump(imgs_dict, f,indent=2)
        if name and self._wandb_logger:
    ...
    def calc_metric_imgs(
        ...
        save_path = self.get_save_path(filename)
        img_dir = os.path.join(self.get_save_dir(), img_dir)
        return calc_metric_imgs_(save_path,img_dir,matcher,criterions,name)

...
def get_ray_directions(
    ...
    if type(cx) == torch.Tensor:
        directions: Float[Tensor, "H W 3"] = torch.stack(
            [(i.to(cx) - cx) / fx, -(j.to(cx) - cy) / fy, -torch.ones_like(i).to(cx)], -1
        )
    else:
        directions: Float[Tensor, "H W 3"] = torch.stack(
...
# code adapted from https://strawlab.org/2011/11/05/augmented-reality-with-OpenGL/ with window_coords=='y up'
def get_projection_matrix_from_intrinsic(
    intrinsic: Float[Tensor, "B 4 4"], h: float, w: float, near: float, far: float
) -> Float[Tensor, "B 4 4"]:
    proj_mtxs = []
    for K in intrinsic:
        ...
        proj_mtxs.append(proj_mtx)
    proj_mtxs=torch.stack(proj_mtxs)
    return proj_mtxs
...
def cylin2cart(
        elevation: Float[Tensor, "N"],
        azimuth: Float[Tensor, "N"],
        distances: Float[Tensor, "N"],
        rad = False, # angles represented in 'radius' or not (i.e. in 'degree')
    ) -> Float[Tensor, "N 3"]:
        """ convert position from Cylindrical (elevation, azimuth, distance) to Cartesian system (x,y,z)"""
        
        if not rad:
            elevation = elevation/180*np.pi
            azimuth = azimuth/180*np.pi

        x = distances * torch.cos(elevation) * torch.cos(azimuth)
        y = distances * torch.cos(elevation) * torch.sin(azimuth)
        z = distances * torch.sin(elevation)

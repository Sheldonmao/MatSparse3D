...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        if self.split == "val":
            ...
            elevation_deg = torch.linspace(0, self.cfg.eval_elevation_deg, self.n_views //2)
            elevation_deg = torch.cat([elevation_deg,torch.linspace(self.cfg.eval_elevation_deg, 0, self.n_views - self.n_views//2 + 1)[: -1]])
        else:
            azimuth_deg = torch.linspace(0, 360.0, self.n_views)
            elevation_deg = torch.linspace(0, self.cfg.eval_elevation_deg, self.n_views//2)
            elevation_deg = torch.cat([elevation_deg,torch.linspace(self.cfg.eval_elevation_deg, 0, self.n_views - self.n_views//2)])
        # elevation_deg: Float[Tensor, "B"] = torch.full_like(
        #     azimuth_deg, self.cfg.eval_elevation_deg
        # )
        # print(f"elevation_deg.shape {elevation_deg.shape}")

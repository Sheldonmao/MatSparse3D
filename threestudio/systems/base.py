...
class BaseLift3DSystem(BaseSystem):
    ...
    class Config(BaseSystem.Config):
        ...
        light_convert_from: Optional[str] = None
        material_convert_from: Optional[str] = None
    ...
    def configure(self) -> None:
        ...
        ## load material if given
        if (
            self.cfg.material_convert_from  # from_coarse must be specified
            and not self.cfg.weights  # not initialized from coarse when weights are specified
            and not self.resumed  # not initialized from coarse when resumed from checkpoints
        ):
            state_dict_material, _, _ = load_module_weights(
                self.cfg.material_convert_from,
                module_name="material",
                map_location="cpu",
            )
            self.material.load_state_dict(state_dict_material)
        
        ## load light if given
        if (
            self.cfg.light_convert_from  # from_coarse must be specified
            and not self.cfg.weights  # not initialized from coarse when weights are specified
            and not self.resumed  # not initialized from coarse when resumed from checkpoints
            and hasattr(self.renderer,"env_light")
        ):
            # TODO: save hdr for loading
            state_dict_renderer, _, _ = load_module_weights(
                self.cfg.light_convert_from,
                module_name="renderer",
                map_location="cpu",
            )
            self.renderer.load_env_base(state_dict_renderer["env_light.env_base"])

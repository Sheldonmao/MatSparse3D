...
@threestudio.register("neuspir-volume-renderer")
class NeuSPIRVolumeRenderer(VolumeRenderer):
    @dataclass
    ...
    def forward(
        ...
        **kwargs
        ...
        if env_light==None:
            self.env_light.build_mips() # important to update mips for each batch
        ...
        comp_values_fg: Float[Tensor, "Nr Nc"] = nerfacc.accumulate_along_rays(
            weights[..., 0], values=rgb_fg_all, ray_indices=ray_indices, n_rays=n_rays
        )

        # unravel the values for rgb, kd, roughness, metallic and occlusion
        comp_rgb_fg = comp_values_fg[...,:3]
        comp_kd_fg = comp_values_fg[...,3:6]
        comp_ks_fg = comp_values_fg[...,6:9]
        comp_sem_feat_fg = comp_values_fg[...,9:]
        ...
        # bg_values = torch.rand_like(comp_values_fg[:,:3]) # create random background with shape (Nr, 3)
        bg_values = torch.ones_like(comp_values_fg[:,3:]) # create random background with shape (Nr, 3)
        comp_kd = comp_kd_fg + bg_values[:,0:3] * (1.0 - opacity)
        comp_ks = comp_ks_fg + bg_values[:,3:6] * (1.0 - opacity)
        comp_sem_feat = comp_sem_feat_fg + bg_values[:,6:] * (1.0 - opacity)

        comp_sem_feat = torch.nn.functional.normalize(comp_sem_feat,dim=-1)
        ...
        out = {
            ...
            "comp_kd": comp_kd.view(batch_size, height, width, -1),
            "comp_ks": comp_ks.view(batch_size, height, width, -1),
            "comp_sem_feat": comp_sem_feat.view(batch_size, height, width, -1),
        }
